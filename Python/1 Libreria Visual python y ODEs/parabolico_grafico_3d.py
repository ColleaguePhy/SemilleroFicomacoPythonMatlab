...
## Creamos el 'suelo'
suelo = vs.box(pos=vs.vector(x_final/2., -1, 0), size = vs.vector(x_final, 1, 10), color = vs.color.green)
## Creamos el 'cañón'
canyon = vs.cylinder(pos = vs.vector(0, 0, 0),
                     axis = vs.vector(2 * np.cos(np.deg2rad(alfa)), 2 * np.sin(np.deg2rad(alfa)), 0))
...
flecha = vs.arrow(pos = vs.vector(0, 0, 0),
                  axis = vs.vector(v0x, v0z, 0), color = vs.color.yellow)
...
t = 0
while t <= t_total:
    bola.pos = vs.vector(v0x * t, v0z * t - 0.5 * g * t**2, 0)
    flecha.pos = vs.vector(v0x * t, v0z * t - 0.5 * g * t**2, 0)
